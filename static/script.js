...
async function showOverview() {
    ...
    content.innerHTML = '<div class="loading"><span class="loading-spinner"></span> Generating overview...</div>';
    ...
    try {
        ...
        if (data.success) {
            ...
            const metaParts = [];
            if (typeof counts.emails === 'number') metaParts.push(`${counts.emails} emails`);
            if (typeof counts.events === 'number') metaParts.push(`${counts.events} events`);
            if (typeof counts.reminders === 'number') metaParts.push(`${counts.reminders} reminders`);
            if (typeof counts.messages === 'number') metaParts.push(`${counts.messages} messages`);
            if (data.model_used) metaParts.push(`Model: ${data.model_used}`);
            ...
            content.innerHTML = `
                ...
                <div class="summary-body">${safeSummary}</div>
            `;
        } else {
            content.innerHTML = `<div class="empty-state">${escapeHtml(data.error || 'Failed to generate overview')}</div>`;
        }
    } catch (error) {
        console.error('Error generating overview:', error);
        content.innerHTML = '<div class="empty-state">Error generating overview</div>';
    }
}
